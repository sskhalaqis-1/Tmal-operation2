    ...
    return {
        ...
        "checklistTasks": {
            "opening": [
                ...
                "Check grinder hoppers and bean levels",
                "Turn on espresso machine and check pressure",
                "Prepare milk station and backup pitchers",
                "Check fridge temperature",
                "Set up cups, lids, sleeves, and bags",
            ],
            "closing": [
                "Backflush espresso machine",
                "Clean grinders and counters",
                "Restock bar supplies for opening",
                "Empty bins and wipe floor area",
                "Check fridges are closed and labeled",
                "Submit bar photo before leaving",
            ],
            "cleaning": [
                "Clean steam wands",
                "Sanitize work surfaces",
                "Wash knock box and drip trays",
                "Clean dessert display",
                "Sweep and mop bar area",
            ],
            "machine": [
                "Espresso machine ready",
                "Grinders calibrated",
                "Water filter pressure normal",
                "POS and receipt printer working",
            ],
        },
        "inventoryItems": [
            {"id": "lemon_cake", "name": "Lemon Cake", "name_ar": "كيكة ليمون", "category": "Desserts", "unit": "pcs"},
            {"id": "chocolate_cake", "name": "Chocolate Cake", "name_ar": "كيكة شوكلت", "category": "Desserts", "unit": "pcs"},
            {"id": "date_cake", "name": "Date Cake", "name_ar": "كيكة تمر", "category": "Desserts", "unit": "pcs"},
            {"id": "swiss_roll", "name": "Swiss Roll", "name_ar": "سويس رول", "category": "Desserts", "unit": "pcs"},
            {"id": "cinnamon_rolls", "name": "Cinnamon Rolls", "name_ar": "سينامون رولز", "category": "Desserts", "unit": "pcs"},
            {"id": "classic_cookies", "name": "Classic Cookies", "name_ar": "كوكيز كلاسيك", "category": "Desserts", "unit": "pcs"},
            {"id": "chocolate_cookies", "name": "Chocolate Cookies", "name_ar": "كوكيز شوكلت", "category": "Desserts", "unit": "pcs"},
        ],
        "issueTypes": [
            "Shortage",
            "Expired product",
...
def load_config() -> dict:
    ...
    config.setdefault("branches", defaults["branches"])
    config.setdefault("issueTypes", defaults["issueTypes"])
    config.setdefault("inventoryItems", defaults["inventoryItems"])
    legacy_inventory_ids = {"beans_house", "beans_filter", "milk_full", "cups_8oz", "date_labels"}
    current_inventory_ids = {item.get("id") for item in config.get("inventoryItems", []) if isinstance(item, dict)}
    if not current_inventory_ids or current_inventory_ids & legacy_inventory_ids:
        config["inventoryItems"] = defaults["inventoryItems"]
    config.setdefault("checklistTasks", {})
    for key, value in defaults["checklistTasks"].items():
        config["checklistTasks"].setdefault(key, value)
    return config
...
def ensure_runtime_files() -> None:
    ...
    if DEPLOY_DATA_DIR and not CONFIG_FILE.exists():
        ...
        if source_config.exists():
            write_json(CONFIG_FILE, read_json(source_config, default_config()))
        else:
...
class CoffeeOpsHandler(BaseHTTPRequestHandler):
    ...
    def serve_static(self, request_path: str):
        file_name = "index.html" if request_path == "/" else request_path.lstrip("/")
        candidates = [ROOT / file_name, APP_DIR / file_name]
        path = next((candidate for candidate in candidates if candidate.exists() and candidate.is_file()), candidates[0])
        if not path.exists() or not path.is_file():
            self.send_error(404)
            return
